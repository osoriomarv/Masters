...
melt_mm = np.array([28.19+16, 24.31+16, 55.85+16, 26.98+16*1.5, 40.08+16, 58.59+16])
melt_mols_i = melt_wt/melt_mm
melt_mol_frac_i = melt_mols_i/sum(melt_mols_i)

# Fe, Ni, O, Si
metal_wt = np.array([85.89, 5.00, 0, 9.17])
metal_mm = np.array([55.85, 58.59, 16, 28.19])
...
bulk_cat = np.array([melt_mols_i[0]+metal_mols_i_scaled[3], melt_mols_i[1], melt_mols_i[2]+metal_mols_i_scaled[0],
                     melt_mols_i[3], melt_mols_i[4], metal_mols_i_scaled[1]])
# bulk O
bulk_O = melt_mols_i[0]*2 + melt_mols_i[1] + melt_mols_i[2] + melt_mols_i[3]*1.5 + melt_mols_i[4]

bulk_comp = np.concatenate((bulk_cat, np.array([bulk_O])))
...
#Preallocation
FeO_mol_frac = np.zeros(range(len(FeO_guess_v)))
NiO_mol_frac = np.zeros(range(len(FeO_guess_v)))
SiO2_mol_frac = np.zeros(range(len(FeO_guess_v)))
Fe_mol_frac = np.zeros(range(len(FeO_guess_v)))
Ni_mol_frac = np.zeros(range(len(FeO_guess_v)))
O_mol_frac = np.zeros(range(len(FeO_guess_v)))
Si_mol_frac = np.zeros(range(len(FeO_guess_v)))

KD_O_guess = np.zeros(range(len(FeO_guess_v)))
iternum = np.zeros(range(len(FeO_guess_v)))
...
e=1
for w in range(1, len(FeO_guess_v) - 1):
...
[min_comp, min_pos] = min(abs(KD_O_guess - KD_O_v[q]))
